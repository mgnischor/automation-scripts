...
#--------------------------------------------------------------------------------------------------
# File: /src/linux/red_hat_hardening.sh
# Description: This script applies basic hardening measures specific to Red Hat-based systems
...
install_security_packages() {
    ...
    yum install -y firewalld fail2ban audit aide libpwquality
    if [ $? -eq 0 ]; then
...
configure_firewall() {
    ...
    firewall-cmd --zone=drop --add-port=$RANDOM_PORT/tcp --permanent
    firewall-cmd --reload
...
harden_ssh() {
    ...
    if [ -f "$SSH_CONFIG" ]; then
        ...
        if command -v semanage &> /dev/null; then
            semanage port -a -t ssh_port_t -p tcp $RANDOM_PORT 2>/dev/null || semanage port -m -t ssh_port_t -p tcp $RANDOM_PORT
        fi
...
configure_audit() {
    ...
    if command -v auditctl &> /dev/null; then
        ...
        auditctl -w /etc/sysconfig/network-scripts/ -p wa -k network_changes
...
configure_password_policy() {
    ...
    PAM_PWD="/etc/pam.d/system-auth"
    if [ -f "$PAM_PWD" ]; then
        # Backup original
        cp "$PAM_PWD" "${PAM_PWD}.bak"
        # Set password complexity requirements
        if ! grep -q "pam_pwquality.so" "$PAM_PWD"; then
            sed -i '/^password.*requisite.*pam_pwquality.so/d' "$PAM_PWD"
            sed -i '/^password.*required.*pam_unix.so/i password    requisite     pam_pwquality.so retry=3 minlen=14 difok=3 ucredit=-1 lcredit=-1 dcredit=-1 ocredit=-1 maxrepeat=3 gecoscheck=1' "$PAM_PWD"
        fi
    fi
    # Set password aging
...
disable_unnecessary_services() {
    ...
    SERVICES=("telnet" "rsh" "rlogin" "vsftpd" "httpd" "cups" "avahi-daemon" "bluetooth")
...
configure_selinux() {
    ...
    if command -v getenforce &> /dev/null; then
        ...
        if [ "$SELINUX_STATUS" != "Enforcing" ]; then
            ...
            echo "SELinux set to enforcing mode. Reboot required to apply."
        else
...
# Function to harden GRUB bootloader
harden_grub() {
    echo "=== Hardening GRUB Bootloader ==="
    GRUB_CFG="/etc/grub.d/40_custom"
    if [ -f "$GRUB_CFG" ]; then
        # Generate password hash (interactive)
        echo "Setting GRUB password protection..."
        GRUB_PASS=$(echo -e "password\npassword" | grub2-mkpasswd-pbkdf2 | grep "grub.pbkdf2" | awk '{print $NF}')
        if [ ! -z "$GRUB_PASS" ]; then
            echo "set superusers=\"root\"" >> "$GRUB_CFG"
            echo "password_pbkdf2 root $GRUB_PASS" >> "$GRUB_CFG"
            grub2-mkconfig -o /boot/grub2/grub.cfg 2>/dev/null || true
            echo "GRUB password protection enabled."
        fi
    fi
}
...
echo ""
echo "========================================="
echo "Red Hat hardening script completed."
